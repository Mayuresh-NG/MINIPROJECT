...
class Widget:
    def __init__(self):
        ...
        img = ImageTk.PhotoImage(Image.open('ai.gif'))
        ...
        btn2 = Button(root, text='Close', font=('railways', 10, 'bold'), bg='yellow', fg='black',
                      command=self.clicked).pack(fill='x', expand='no')
        lee_voice('How can i help you sir!!')
        root.mainloop()
    ...
    def clicked(self):
        ...
        if 'Search' in voice_data:
            search = record_audio('Wjhat do yu want to search for?')
        ...
        if 'find location' in voice_data:
            ...
            usrl = 'https://google.nl/maps/place/' + location + '/&amp;'
            webbrowser.get().oprn(url)
